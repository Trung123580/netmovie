    ...
    <div className='container'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4  md:gap-x-5 gap-x-[15px] gap-y-6 md:gap-y-10'>
        {currentUser?.loveMovie.map((movie: any) => (
          <CardProduct device={device} data={movie} key={movie.id} onToggleMovie={() => onToggleMovie(movie)} findIsLoveMovie={currentUser?.loveMovie.some((item: any) => item.id === movie.id)} />
        ))}
      </div>
    </div>
  )
