...
const Nav = ({ onToggleNavbar, isShowNavBar, user, isAuthenticated, onToggleOpenMenuCategory, openMenuCategory, convertHeader, isMobile, onShowPopup, onLoginGG, onAppSignOut }: navTypes) => {
      ...
      <ul className={`${isShowNavBar ? "block" : "hidden"} z-50  lg:flex lg:gap-6 fixed lg:static text-black bg-overlay lg:bg-transparent top-0 h-full w-80 right-0 lg:w-auto`}>
        {convertHeader.map(({ path, name, id, category, Icon }: any, index: number) => (
          <li key={id} className='group md:py-0 md:px-0 border-b-2 font-medium relative border-b-black lg:border-b-transparent text-white md:text-black cursor-default hover:bg-black lg:hover:bg-transparent '>
            ...
            {!!category.length && (
              <ul
                className={`${isShowNavBar && `!static !grid-cols-2 !w-full ${openMenuCategory === path ? "!grid" : "!hidden"}`} ${path === "/loai-phim" ? "!grid-cols-2 !-left-[90%]" : ""} ${
                  path === "/top-phim" ? "!py-0" : ""
                } min-h-[450px] h-[450px] overflow-auto ${index === 0 ? "!h-auto !min-h-min" : ""} group-hover:grid grid dropdown-menu absolute duration-300 !px-4 grid-cols-2 top-full -left-[180%] z-50 text-sm text-white bg-blur`}>
        ...
        <Button
          ...
          icon={
            <abbr title={`${isAuthenticated ? "thông tin tài khoản" : "Đăng nhập"}`}>
              <img loading='lazy' src={user?.photoURL} width={23} height={23} className='rounded min-w-[23px]' alt='user' />
            </abbr>
          }
          className='text-black cursor-pointer hover:opacity-90 flex '></Button>
      ) : (
        <Button
          onClick={onLoginGG}
          icon={
